...
fn (mut pv Picodb) update_events(fd int, events int) int {
	...
	pv.file_descriptors[fd].events = u32(events & picodb_readwrite)
	return 0
}
...
fn (mut pv Picodb) poll_once(max_wait int) int {
	...
	for target in pv.file_descriptors {
		if target.loop_id == pv.loop.id {
			if target.events & picodb_read != 0 {
				C.FD_SET(target.fd, &readfds)
				if maxfd < target.fd {
					maxfd = target.fd
				}
			}
			if target.events & picodb_write != 0 {
	...
	if r == -1 {
		...
		for target in pv.file_descriptors {
			if target.loop_id == pv.loop.id {
				// vfmt off
				read_events := (
					(if C.FD_ISSET(target.fd, &readfds) { picodb_read } else { 0 })
						|
					(if C.FD_ISSET(target.fd, &writefds) { picodb_write } else { 0 })
				)
				// vfmt on
				if read_events != 0 {
					$if trace_fd ? {
