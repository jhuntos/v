...
[direct_array_access]
fn (mut pv Picodb) update_events(fd int, events int) int {
	...
	// fd belongs to loop
	if events & picodb_del != target.events && target.loop_id != pv.loop.id {
		return -1
	}

	if events & picodb_readwrite == target.events {
		return 0
	}

	// vfmt off
	ev.events = u32(
		(if events & picodb_read != 0 { C.EPOLLIN } else { 0 })
			|
		(if events & picodb_write != 0 { C.EPOLLOUT } else { 0 })
	)
	// vfmt on
	ev.data.fd = fd

	if events & picodb_del != 0 {
		// nothing to do
	} else if events & picodb_readwrite == 0 {
...
[direct_array_access]
fn (mut pv Picodb) poll_once(max_wait int) int {
	...
	for i := 0; i < nevents; i++ {
		...
		if pv.loop.id == target.loop_id && target.events & picodb_readwrite != 0 {
			// vfmt off
			read_events := (
				(if event.events & u32(C.EPOLLIN) != 0 { picodb_read } else { 0 })
					|
				(if event.events & u32(C.EPOLLOUT) != 0 { picodb_write } else { 0 })
			)
			// vfmt on
			if read_events != 0 {
				// do callback!
