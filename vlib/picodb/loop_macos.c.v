...
[inline]
pub fn (mut pv Picodb) ev_set(index int, operation int, events int) {
	// vfmt off
	filter := i16(
		(if events & picodb_read != 0 { C.EVFILT_READ } else { 0 })
			|
		(if events & picodb_write != 0 { C.EVFILT_WRITE } else { 0 })
	)
	// vfmt on
...
fn (mut pv Picodb) apply_pending_changes(apply_all bool) int {
...
fn (mut pv Picodb) update_events(fd int, events int) int {
	...
	if events & picodb_add != 0 {
		target.backend = -1
	}

	// return if nothing to do
	if (events == picodb_del && target.backend == -1)
		|| (events != picodb_del && events & picodb_readwrite == target.events) {
		return 0
	}
	...
	// update events
	target.events = u32(events & picodb_readwrite)
