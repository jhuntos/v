...
pub const (
	...
	picopg_write     = 2
	picopg_timeout   = 4
	picopg_add       = 0x40000000
	picopg_del       = 0x20000000
	picopg_readwrite = 3 // 1 xor 2
)
...
pub mut:
	port         int = 8080
	cb           fn (voidptr, picohttpparser.Request, mut picohttpparser.Response, pg.DB, mut []Routes)
...
pub fn (mut pv Picopg) init() {
	assert picopg.max_fds > 0

	pv.num_loops = 0

	for i in 0 .. picopg.max_fds {
...
[direct_array_access]
pub fn (mut pv Picopg) add(fd int, events int, timeout int, cb voidptr) int {
	assert fd < picopg.max_fds
	...
	if pv.update_events(fd, events | picopg.picopg_add) != 0 {
		pv.del(fd)
		return -1
	}
...
[direct_array_access]
fn (mut pv Picopg) del(fd int) int {
	assert fd < picopg.max_fds
	mut target := pv.file_descriptors[fd]
	...
	if pv.update_events(fd, picopg.picopg_del) != 0 {
		target.loop_id = -1
		target.fd = 0
		return -1
	}
...
fn (mut pv Picopg) loop_once(max_wait int) int {
...
fn (mut pv Picopg) set_timeout(fd int, secs int) {
	assert fd < picopg.max_fds
	if secs != 0 {
...
// timeout event
[direct_array_access; inline]
fn (mut pv Picopg) handle_timeout() {
	for fd, timeout in pv.timeouts {
		if timeout <= pv.loop.now {
			...
			pv.timeouts.delete(fd)
			unsafe { target.cb(fd, picopg.picopg_timeout, &pv) }
		}
	}
}
...
fn accept_callback(listen_fd int, events int, cb_arg voidptr) {
	mut pv := unsafe { &Picopg(cb_arg) }
	newfd := accept(listen_fd)
	if newfd >= picopg.max_fds {
		// should never happen
		close_socket(newfd)
		return
	}
	...
	if newfd != -1 {
		...
		pv.add(newfd, picopg.picopg_read, pv.timeout_secs, raw_callback)
	}
}

// close_conn closes the socket `fd` and removes it from the loop
[inline]
pub fn (mut pv Picopg) close_conn(fd int) {
	pv.del(fd)
	close_socket(fd)
}
...
fn raw_callback(fd int, events int, context voidptr) {
	mut pv := unsafe { &Picopg(context) }
	defer {
		pv.idx[fd] = 0
	}

	if events & picopg.picopg_timeout != 0 {
		$if trace_fd ? {
			eprintln('timeout ${fd}')
		}
		pv.close_conn(fd)
		return
	} else if events & picopg.picopg_read != 0 {
...
fn default_err_cb(data voidptr, req picohttpparser.Request, mut res picohttpparser.Response, error IError) {
	eprintln('picopg: ${error}')
	res.end()
}

// new creates a `picopg` struct and initializes the main loop
pub fn new(config Config) &Picopg {
	listen_fd := listen(config)

	dbc := pg.connect(config.config) or { panic(err) }
	...
	mut pv := &Picopg{
		...
		db: &dbc
		rr: config.routes
		buf: unsafe { malloc_noscan(picopg.max_fds * config.max_read + 1) }
		out: unsafe { malloc_noscan(picopg.max_fds * config.max_write + 1) }
	}
	...
	pv.add(listen_fd, picopg.picopg_read, 0, accept_callback)
	return pv
}

// serve starts the picopg server
pub fn (mut pv Picopg) serve() {
